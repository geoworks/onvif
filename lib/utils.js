			...
			if (!result || !result['envelope'] || !result['envelope']['body']) {
				...
				if (!err && result['envelope']['body'][0]['fault']) {
					...
					if (
					    ...
					    result.envelope.body[0].fault &&
					    result.envelope.body[0].fault[0] &&
					    result.envelope.body[0].fault[0].detail &&
					    result.envelope.body[0].fault[0].detail[0]
					  ) {
					    if (
					      result.envelope.body[0].fault[0].detail[0].text &&
					      result.envelope.body[0].fault[0].detail[0].text[0]
					    ) {
					      detail = result.envelope.body[0].fault[0].detail[0].text[0];
					    }
					    if (
					      result.envelope.body[0].fault[0].detail[0].error &&
					      result.envelope.body[0].fault[0].detail[0].error[0]
					    ) {
					      detail = result.envelope.body[0].fault[0].detail[0].error[0];
					    }
					  }
